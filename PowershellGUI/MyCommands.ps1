...
Function Ping-Computer {
    ...
    # Ping the target computer with the Test-Connection command
    $Ping = Test-Connection $Computer
    $Ping
}

# Where is the XAML file?
$xamlFile = "C:\Users\nickb\Desktop\PowershellGUI\MainWindow.xaml"
...
$var_pingQuery.Add_Click( {
    #clear the result box
    $var_txtResults.Text = ""
        # if ($result = Ping-Computer -Computer $var_txtComputer.Text) {
        #   foreach ($item in $result) {
        #       $var_txtResults.Text = $var_txtResults.Text + "Ping: $($item)`n"
        #    }
        #}       

        $var_txtResults.Text = $var_txtResults.Text + "Ping: $(Ping-Computer -Computer $var_txtComputer.Text)`n"
    })
